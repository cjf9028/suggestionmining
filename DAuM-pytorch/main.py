...
parser.add_argument('--dataset', default='data/suggestion/')
#parser.add_argument('--embedding_file_name', default='data/glove.840B.300d.txt')
parser.add_argument('--embedding_file_name', default='data/wiki.en.vec')
...
def main():
    ...
    dev_data = read_data(word2id, config.max_aspect_len, config.max_sentence_len, config.dataset + 'dev',
                          config.pre_processed)
    ...
    dev_dataset = DAuMDataset(dev_data)
    ...
    dev_loader = DataLoader(dataset=dev_dataset, batch_size=config.batch_size, shuffle=False, num_workers=2)
    test_loader = DataLoader(dataset=test_dataset, batch_size=config.batch_size, shuffle=False, num_workers=2)
    ...
    for epoch in range(config.n_epoch):
        ...
        max_dev = max(max_dev, dev_accuracy)
        ...
        for data in test_loader:
            ...
            outputs, _, _ = model(sentences, aspects, sentence_masks, aspect_masks, aspect_positions)
            _, predicts = outputs.max(dim=1)
            with open("result/"+str(start_time)+"epoch_"+str(epoch)+"_result.txt","a")as f:
        ...
        print('[epoch %03d] train accuracy: %.4f dev accuracy: %.4f test accuracy: %.4f' % (epoch,train_accuracy,dev_accuracy, test_accuracy))

        max_acc = max(max_acc, test_accuracy)
    print('max test accuracy: %.4f' % max_acc)
    print('max dev accuracy: %.4f' % max_dev)
    end_time = time.time()
